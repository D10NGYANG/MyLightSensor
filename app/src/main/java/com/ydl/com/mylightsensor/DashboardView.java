...
/**
 * Created by ydl on 2018/4/4 0004.
 */
...
public class DashboardView extends View {
    ...
    private float mMin = 0; // 最小值
    private float mMax = 1000; // 最大值
    private String mHeaderText = "LUX"; // 表头
    private int mCreditValue = 0; //当前值
    private int mSparkleWidth; // 亮点宽度
    private int mProgressWidth; // 进度圆弧宽度
    private float mLength1; // 刻度顶部相对边缘的长度
    private float mLength2; // 值指示器顶部相对边缘的长度
    ...
    private void init() {
        mSparkleWidth = dp2px(10);  // 亮点宽度
        mProgressWidth = 4;   // 进度圆弧宽度
    ...
    private String calculateCreditDescription() {
        if (mCreditValue > 850) {
            return "非常亮";
        } else if ((mCreditValue > X + 20)&&(mCreditValue < 850)) {
            return "较亮";
        } else if ((mCreditValue > X - 20)&&(mCreditValue < X + 20)) {
            return "标准";
        } else if (mCreditValue < X - 20) {
            return "过暗";
        }
        return "过暗";
    }
    ...
    private String getFormatTimeStr() {
        ...
        return String.format("测量时间:%s", mDateFormat.format(new Date()));
    }
